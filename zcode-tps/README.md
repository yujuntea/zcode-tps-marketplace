...
> 会话归属：`/speed` 面板统计你**运行命令的会话**；菜单栏与 `--watch` 终端行是全局单例，跟随**最近有模型活动的会话**（ZCode 切换会话不产生本地信号，插件无法感知界面焦点）——下拉中「会话:」与行尾会话标题标明当前归属；想固定看某个会话用 `--session <id>`。详见根 README「统计的是哪个会话（避免误解）」。
