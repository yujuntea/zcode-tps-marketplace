...
- 数字属于**最近有模型活动的会话**（不是你正在查看的会话）——下拉里「会话:」一行标明归属；在新会话发出第一条消息后，数字数秒内自动跟随过去。详见[「统计的是哪个会话（避免误解）」](#统计的是哪个会话避免误解)
...
> 这行数字与菜单栏同规则：跟随**最近有模型活动的会话**，行尾的「会话标题」标明归属（纯切换 ZCode 会话不会改变它），详见[「统计的是哪个会话（避免误解）」](#统计的是哪个会话避免误解)。
...
三种查看方式的数据**归属会话**不同，先看这张表：

| 查看方式 | 统计哪个会话 | 归属在哪看 |
|---|---|---|
| `/speed` 面板 | 你**运行命令的会话**（`/speed` 本身就是该会话的最新请求，天然自定位） | 面板首行「会话「…」」 |
| 菜单栏 SwiftBar | **最近有模型请求完成的会话** | 下拉中「会话: …」一行 |
| 终端 `--watch` | **最近有模型请求完成的会话** | 行尾「「会话标题」」 |

菜单栏和终端行是**全局单例**，而 ZCode 目前不提供「正在查看哪个会话」的信号——在界面里切换会话不产生任何本地记录（插件只读数据库、无 hook，无从感知）。所以它们跟随**最近活动**而非界面焦点：在哪个会话发出消息、请求完成，数字数秒内就切到哪个会话；纯切换不改变显示。

如果数字和你正在看的会话对不上：先看上表的归属标识确认它属于谁；想固定查看某个会话，用 `/speed --list-sessions` 找到会话 id（前缀唯一即可），再 `/speed --session <id>` 打开该会话的面板。
...
- 会话定位 = 最近完成请求所在的根会话（沿 `parent_id` 递归上溯）；子代理会话按树聚合（`/speed` 面板等价于统计运行命令的会话，见[「统计的是哪个会话」](#统计的是哪个会话避免误解)）
...
- 菜单栏/终端行跟随**最近有模型活动的会话**，不跟随 ZCode 界面正在查看的会话（切换会话无本地信号，插件无法感知，详见[「统计的是哪个会话」](#统计的是哪个会话避免误解)）；指定会话请用 `--session <id>`
